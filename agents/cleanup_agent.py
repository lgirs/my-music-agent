...
def handle_album_action(artist, album_title, album_id, action_type):
    ...
    try:
        ...
        # 2. Robust Removal: Scan playlist and remove by INDEX (more reliable)
        # First, fetch the target album to get its track IDs
        target_album = tidal_client.session.album(album_id)
        target_tracks = target_album.tracks()
        target_track_ids = set(t.id for t in target_tracks)
        ...
        # Now scan the playlist items
        playlist_items = playlist.items()
        indices_to_remove = []
        
        # We iterate backwards to avoid index shifting issues during removal,
        # though tidalapi usually handles lists of indices well.
        for i, item in enumerate(playlist_items):
            # Check if the item is a track and if its ID matches our target album's tracks
            if item.item and item.item.id in target_track_ids:
                indices_to_remove.append(i)
        
        if indices_to_remove:
            print(f"  > Found {len(indices_to_remove)} matching tracks in playlist at indices: {indices_to_remove}")
            
            # Delete by index is the most robust method
            # Note: We must sort indices descending to avoid shifting problems if the API processes sequentially
            indices_to_remove.sort(reverse=True)
            
            # Depending on the tidalapi version, we might need to remove one by one or batch.
            # Batch removal by index is safer if supported, but loop is safest for now.
            for index in indices_to_remove:
                playlist.remove_by_index(index)
                
            print(f"  > Successfully removed tracks from '{PLAYLIST_NAME}'.")
        else:
            print(f"  > Warning: Album tracks not found in '{PLAYLIST_NAME}'. Already removed?")
